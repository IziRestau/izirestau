...
import { NewTicketModal } from '@/components/reseller/support/NewTicketModal'

type StatusFilter = 'all' | 'OPEN' | 'IN_PROGRESS' | 'RESOLVED' | 'CLOSED'
type CategoryFilter = 'all' | 'BILLING' | 'TECHNICAL' | 'FEATURE_REQUEST' | 'ACCOUNT' | 'OTHER'
...
export default function SupportPage() {
  ...
  return (
    ...
    >
      <PageHeader
        title="Support"
        subtitle="Contactez l'equipe IziResto"
        icon={HelpCircle}
        actions={
        ...
        {filteredTickets.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100">
            {filteredTickets.map((ticket) => (
              <button
                key={ticket.id}
                onClick={() => router.push(`/reseller/support/${ticket.id}`)}
