...
export default function AccountPage() {
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      router.push(`/store/${subdomain}/login?redirect=/store/${subdomain}/account`)
    }
  }, [isLoading, isAuthenticated, router, subdomain])
  ...
  const AccountPageComponent = themeComponents.AccountPage
  if (!AccountPageComponent) {
    return (
      <StorefrontLayout>
        <div className="min-h-[60vh] flex items-center justify-center">
          <p className="text-gray-500">Page de compte non disponible</p>
        </div>
      </StorefrontLayout>
    )
  }
  ...
  return (
    <StorefrontLayout>
      <AccountPageComponent
        ...
        sections={storeData.pages?.account?.sections}
        subdomain={subdomain}
