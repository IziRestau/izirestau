      ...
      <section className={`py-10 sm:py-14 ${isFloatingHeader ? 'pt-24 sm:pt-28' : ''}`} style={{ backgroundColor: theme.backgroundColor }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            {showRefreshButton && (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 text-sm font-medium transition-all ${btnClass}`}
          ...
          <div className="text-center">
            <div 
              className={`inline-flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 mb-4 ${btnClass}`}
              style={{ backgroundColor: isCancelled ? '#fee2e2' : `${theme.primaryColor}15` }}
            >
              <StatusIcon 
                size={40} 
                style={{ color: isCancelled ? '#ef4444' : theme.primaryColor }} 
                className={isCancelled || isCompleted ? '' : 'animate-pulse'} 
              />
            </div>
            <h1 
              className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-2"
              style={{ fontFamily: `'${theme.headingFont}', sans-serif`, color: isCancelled ? '#ef4444' : theme.textColor }}
            >
              {statusConfig.label}
            </h1>
            <p 
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
            {showOrderNumber && (
              showCopyButton ? (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 text-sm transition-all hover:opacity-80 ${btnClass}`}
                  style={{ backgroundColor: `${theme.textColor}08`, color: theme.textColor }}
                >
                ...
                <span 
                  className={`px-4 py-2.5 text-sm ${btnClass}`}
                  style={{ backgroundColor: `${theme.textColor}08`, color: theme.textColor }}
                >
                  <span className="opacity-60">Commande</span>{' '}
                  <span className="font-mono font-bold">#{order.displayNumber || order.orderNumber}</span>
            ...
            {showEstimatedTime && !isCompleted && !isCancelled && order.estimatedTime > 0 && (
              <span 
                className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium ${btnClass}`}
                style={{ backgroundColor: `${theme.primaryColor}15`, color: theme.primaryColor }}
              >
                <Clock size={16} />
        ...
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center justify-between mb-8">
            {showBackButton ? (
              <Link 
                href={`/store/${subdomain}/menu`}
                className={`flex items-center gap-2 px-4 py-2 bg-white/15 hover:bg-white/25 backdrop-blur-sm text-white text-sm font-medium transition-all ${btnClass}`}
              >
                <ArrowLeft size={18} />
                <span>{backButtonText}</span>
              </Link>
          ...
          <div className="text-center">
            <div 
              className={`inline-flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 bg-white/20 backdrop-blur-sm mb-4 ${btnClass}`}
            >
              <StatusIcon 
                size={40} 
                className={`text-white ${isCancelled || isCompleted ? '' : 'animate-pulse'}`} 
              />
            </div>
            <h1 
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
            {showOrderNumber && (
              showCopyButton ? (
                <button
                  onClick={copyOrderNumber}
                  className={`flex items-center gap-2 px-4 py-2.5 bg-white/15 hover:bg-white/25 backdrop-blur-sm text-white text-sm transition-all ${btnClass}`}
                >
                ...
                <span className={`px-4 py-2.5 bg-white/15 backdrop-blur-sm text-white text-sm ${btnClass}`}>
                  <span className="text-white/60">Commande</span>{' '}
                  <span className="font-mono font-bold">#{order.displayNumber || order.orderNumber}</span>
                </span>
              )
            ...
            {showEstimatedTime && !isCompleted && !isCancelled && order.estimatedTime > 0 && (
              <span className={`flex items-center gap-2 px-4 py-2.5 bg-white/15 backdrop-blur-sm text-white text-sm font-medium ${btnClass}`}>
                <Clock size={16} />
                <span className="font-bold">{order.estimatedTime} min</span>
              </span>
            )}
