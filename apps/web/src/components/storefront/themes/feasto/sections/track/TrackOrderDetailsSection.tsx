...
export function TrackOrderDetailsSection({
  ...
  const defaultExpanded = s('defaultExpanded', true) === true
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              <div 
                className={`w-10 h-10 flex items-center justify-center ${btnClass}`}
                style={{ backgroundColor: `${theme.primaryColor}15` }}
              >
                <Receipt size={18} style={{ color: theme.primaryColor }} />
              </div>
              <span 
                className="font-semibold"
                style={{ fontFamily: `'${theme.headingFont}', sans-serif`, color: theme.textColor }}
              >
                Détails ({order.items.length} article{order.items.length > 1 ? 's' : ''})
              </span>
            </div>
            <div className="flex items-center gap-3">
              ...
              {isExpanded ? (
                <ChevronUp size={20} style={{ color: `${theme.textColor}40` }} />
              ) : (
                <ChevronDown size={20} style={{ color: `${theme.textColor}40` }} />
              )}
            </div>
          </button>
          ...
          {isExpanded && (
            <div style={{ borderTop: `1px solid ${theme.textColor}10` }}>
                ...
                {order.items.map((item) => (
                  <div key={item.id} className="flex justify-between items-start">
                    <div className="flex items-start gap-3">
                      <span 
                        className={`w-7 h-7 flex items-center justify-center text-xs font-bold text-white flex-shrink-0 ${btnClass}`}
              ...
              >
                {showSubtotal && (
                  <div className="flex justify-between items-center text-sm mb-2 opacity-60" style={{ color: theme.textColor }}>
