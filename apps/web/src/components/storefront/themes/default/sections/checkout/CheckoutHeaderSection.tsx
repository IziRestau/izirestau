...
interface CheckoutHeaderSectionProps {
...
export function CheckoutHeaderSection({
  ...
  theme,
  sectionData,
}: CheckoutHeaderSectionProps) {
  const s = (key: string, fallback?: unknown): unknown => sectionData?.[key] ?? fallback

  const bgType = (s('bgType', 'color') as string)
  const title = (s('title', 'Finaliser votre commande') as string)
  const subtitle = (s('subtitle', 'Plus que quelques étapes pour savourer vos plats') as string)
  ...
  if (bgType === 'gradient') {
    return (
      <section
        className={`py-10 sm:py-14 px-4 sm:px-6 text-center ${floatPad}`}
