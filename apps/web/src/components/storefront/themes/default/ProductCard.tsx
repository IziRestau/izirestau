...
export function ProductCard({ product, theme, settings, onClick, sectionOverrides }: ProductCardProps) {
  ...
  const renderAddButton = (size: 'sm' | 'md') => {
    ...
    if (effectiveAddBtnStyle === 'text') {
      return (
        <span
          className={`inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-white ${btnClass}`}
    ...
    if (effectiveAddBtnStyle === 'both') {
      return (
        <span
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white ${btnClass}`}
          style={{ backgroundColor: theme.primaryColor }}
        >
  ...
  if (effectiveCardStyle === 'horizontal') {
    ...
    return (
      <button
        ...
        style={{ borderColor: effectiveCardBorder ? `${theme.textColor}10` : undefined, backgroundColor: theme.backgroundColor }}
      >
        {!isRight && imageEl}
        <div className="flex-1 min-w-0">
          ...
          {effectiveShowDescriptions && product.description && (
            <p className={`text-xs sm:text-sm mt-1 line-clamp-${effectiveDescLines} opacity-60`} style={{ color: theme.textColor }}>
              {product.description}
            </p>
          )}
          {effectiveShowPrices && (
            <div className="flex items-center gap-2 mt-2">
              ...
              {hasDiscount && (
                <span className="text-xs line-through opacity-40" style={{ color: theme.textColor }}>
    ...
    return (
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          {effectiveShowDescriptions && product.description && (
            <p className={`text-xs mt-0.5 opacity-50 line-clamp-${Math.min(effectiveDescLines, 1)}`} style={{ color: theme.textColor }}>
              {product.description}
            </p>
          )}
        </div>
  ...
  return (
    <button
      ...
      style={{ borderColor: effectiveCardBorder ? `${theme.textColor}08` : undefined, backgroundColor: theme.backgroundColor }}
    >
      ...
      <div className="p-3 sm:p-4">
        ...
        <div className="flex items-center gap-2 mt-2">
          {theme.showPrepTime && product.prepTime && (
            <span className="inline-flex items-center gap-1 text-xs opacity-50" style={{ color: theme.textColor }}>
              <Clock size={12} />
